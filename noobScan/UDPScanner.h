...
/*
 ...
 UDPScanner.h is in charge of the functions pertaining to a UDP Scanner. It scans ports based on input, and outputs to user the open and closed ports encountered. Of note, is that UDP Scanner will function differently based on root privileges of the user. When user is root, UDPScanner will utilize ICMP packets to help determine if a port is open or closed after a timeout period has been reached. With a normal UDP scan, this is not possible (you would need to send various packets and check to see if anything was received a few times. Even then, the lack of response with the port would not guarantee accuracy). Without root, a user would not be able to utilize ICMP packets. On a personal note: when I decided to go with UDP scanning, I thought it would be the easier of the 2 scans to implement well. This was not the case, due to the unreliable nature of UDP compared to TCP.
 Author: John Atti
 Finished 8/16/2020
*/
/**/
